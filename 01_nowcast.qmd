...
class TFT(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X:dict, y:dict):
        ...
        highest_freq_X_ = self._highest_freq(X.keys(), y.keys())
        ...
        resampled_y_df = y_df.resample(highest_freq_X).bfill()
        self.resampled_y_dates = resampled_y_df.index
        self.nowcasting_steps_ = self._count_nowcasting_days(highest_freq_X, list(y.keys())[0])
        ...
        self.split_dates_ = {f"fold_{i}": {"train": train, "valid": test} for i, (train, test) in enumerate(self.tscv.split(resampled_y_df.index))}
        ...
        fit_data = {}
        for fold in tqdm(self.split_dates_.keys(), disable=~self.verbose):
            fit_data[fold] = {}
            for chunk in ["train", "valid"]:
                fit_data[fold][chunk] = {"X": {}, "y": resampled_y_df.iloc[tft.split_dates_[fold][chunk]]}
                for Xk, Xv in X.items():
                    fit_data[fold][chunk]["X"][Xk] = []
                    for y_date in resampled_y_df.index[self.split_dates_[fold][chunk]]:
        ...
        # TODO: build model ---- self.model = self._build_model()
        # TODO: construct tscv
        # TODO: actually fit model

tft = TFT()
tmp_out = tft.fit(X={"D": df_oil_d, "W": df_oil_d.resample("W").bfill(), "QE": df_oil_d.resample("QE").bfill()}, y={"ME": df_infl_m})
tmp_out
```
