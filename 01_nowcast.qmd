...
def create_data(X, y, maxlags=maxlags, tscv=TimeSeriesSplit(n_splits=5)):
    X_split = {}
    y_split = {}
    if tscv:
        ...
        for fold, split in enumerate(cv_dates):
            dates_split = {
                "train": split[0],
                "valid": split[1]
            }
...
for key in y_train_split.keys():
    print(f"Fold No {key+1}:")
    print(f"  {len(y_train_split[key]['train'])} training X-y pairs")
    print(f"  {len(y_train_split[key]['valid'])} validation X-y pairs")
```
...
inputs = [keras.layers.Input(shape=(None, 1), name=f) for f in freqs]
all_inputs = keras.layers.Concatenate(name="continuous")(inputs)
...
keras.utils.plot_model(nn_fc, show_shapes=True)
```
...
# because this network will only take one data at a time
X_train_split_fc = [
    np.array([
        np.squeeze(v)[-1]
        for v in d.values()
    ])
    for d in X_train_split[0]["train"]
]

nn_fc.fit(x=X_train_split_fc, y=y_train_split[0]["train"], epochs=4)
...
keras.utils.plot_model(nn_lstm, show_shapes=True)
```
...
```{python}
#| label: check lstm model works

maxlag = {"m": 12, "d": 250} 
"""
#lstm_x = [
#    {   
#        k: (v if v is not None else pd.Series(0.0)) 
#        for k, v in c[0].items()
#    }  # Replace None values with pd.Series(0.0)
#    for c in train_data["fold_0"]['Xy_train'].values()  # Iterate over values
#]

lstm_x = {
    k: keras.utils.pad_sequences([v], maxlen=maxlag[k])
    for k, v in c[0].items()
    for c in train_data["fold_0"]['Xy_train'].values()
}

lstm_y = [c[1] for c in train_data["fold_0"]['Xy_train'].values()]
"""
"""# Example usage
maxlag = {'m': 12, 'd': 365}  # Define the maxlen for each frequency (monthly, daily)

def pad_sequences_by_frequency(train_data, maxlag):
    formatted_data = []
    
    x_by_freq = {k: [] for k in maxlag.keys()}
    # Iterate over each sample in 'Xy_train'
    for sample, data in train_data["fold_0"]['Xy_train'].items():
        for freq in x_by_freq.keys():       
            x_by_freq[freq].append(data[0][freq])
    
        y_value = data[1]  # The dependent variable y
        # Append the padded X values and y to the formatted data
        formatted_data.append((padded_X, y_value))
    

    # Pad each X based on the frequency
    padded_X = {
        k: keras.utils.pad_sequences([
            v if v is not None else pd.Series(0.0)
        ], maxlen=maxlag[k], padding='post')[0]  # Pad and keep same format
...
# Call the function to pad the sequences by frequency
formatted_train_data = pad_sequences_by_frequency(train_data, maxlag)
"""
{
    k: v
    for xdata in X_train_split[0]["train"]
    for k, v in xdata.items()

}
nn_lstm.fit(x=X_train_split[0]["train"], y=y_train_split[0]["train"], epochs=4)
```
