...
class TFT(BaseEstimator, RegressorMixin):
    ...
    def _build_model(self):
        # all_inputs = 
        all_inputs = {f: keras.layers.Input(shape=(None,1), name=f) for f in self.freq_rank.keys()} # should I change this shape to (None, None)???
        encoded_cont_inputs = {
            k: keras.layers.TimeDistributed(
        ...
        if len(hist_states_h.keys()) > 1:
            ...
            print(f"highest_freq_X_: {self.highest_freq_X_}, hist_LSTMs.keys(): {hist_LSTMs.keys()}")
            ...
            for chunk in ["train", "valid"]:
                ...
                for Xk, Xv in X.items():
                    ...
                    for y_date in y.index[self.split_idx_[fold][chunk]]:
                        ...
                        if self.freq_rank[Xk] > self.freq_rank[self.y_freq_] and date_lim_ydata < y_date:
                            try:
...
fit_data["fold_0"]["train"]["y"]
df_infl_m["1991-06-01":"1991-11-30"]
df_oil_d["1991-08-25":"1991-10-05"]
fit_data["fold_0"]["train"]["X_fut"]["D"][-5:,:,0].T
fit_data["fold_3"]["train"]["X_hist"]["D"][-3,:,:]
fit_data["fold_3"]["train"]["y"].index[-5:]
