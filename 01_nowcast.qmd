...
**Actual TFT implementation**

The model nowcasts the dependent variable at the highest possible frequency. For example, if the variable of interst is monthly inflation and the input data includes daily data, then the output is daily inflation metrics.

The model below implements some changes to facilitate use in datasets of the same size as ecoomic data. In particular,

- the static covariate enrichment is only added when there is more than one entity to nowcast about
...
learning_rate = [1e-4, 1e-3, 1e-2][0]
max_gradient_norm = [0.01, 1.0, 100.0][0]
...
class TFT(BaseEstimator, RegressorMixin):
    ...
    def __init__(
        ...
        compile_args={
            "loss": quantile_loss,
            "optimizer": keras.optimizers.Adam(learning_rate=learning_rate, clipnorm=max_gradient_norm),
            "callbacks": [
        ...
        fit_args = {"epochs": 20, "batch_size":10, "shuffle": True},
        cache_data=True,
        verbose=True
    ):
        self.name = name
    ...
    def _build_model(self):
        # all_inputs = 
        all_inputs = {f: keras.layers.Input(shape=(None,None), name=f) for f in freqs} # should I change this shape to (None, None)???
        encoded_cont_inputs = {
            k: keras.layers.TimeDistributed(
        ...
        state_h = keras.layers.Dense(units=d_model, activation="sigmoid")(state_h)
        ...
        state_c = keras.layers.Dense(units=d_model, activation="sigmoid")(state_c)
        ...
        if len(hist_states_h.keys()) > 1:
            latent = {
                k: keras.Sequential([
                    keras.layers.RepeatVector(num_timesteps_transformer),
                    keras.layers.LSTM(units=d_model, return_sequences=True),
                    keras.layers.TimeDistributed(keras.layers.Dense(units=d_model))
                ], name=f"translate__freq_{k}_to_{self.highest_freq_X_}")(v)
                for k, v in hist_states_h
            ...
            hist_LSTM = keras.layers.TimeDistributed(keras.layers.Dense(units=d_model, use_bias=False))(hist_LSTM)
        else:
        ...
        future_lstm = keras.layers.LSTM(units=self.d_model, return_sequences=True, return_state=False)(future_features, initial_state=[state_h, state_c])
                # out = keras.layers.Dense(units = dim, activation="relu")(encoded_series)
                # out = keras.layers.Dense(units=1)(out)
        ...
        transformer_layer = keras.ops.concatenate([hist_LSTM, future_LSTM], axis=1) 
        ...
        output_lowest_quant = keras.layers.TimeDistributed(
            keras.layers.Dense(self.nowcasting_steps_),
            name=f"output_q{str(self.quantiles[0])}"
        )(transformer_layer[Ellipsis, self.num_encoder_steps:, :])

        output_quant_deltas = [
            keras.layers.TimeDistributed(
                keras.layers.Dense(self.nowcasting_steps_, activation="relu", use_bias=False),
        ...
        output_lowest_quant_expanded = np.expand_dims(output_lowest_quant, axis=-1)
        quantile_outputs = [output_lowest_quant_expanded]
        for delta in output_quant_deltas:
            # Expand delta along the quantile axis and sum with the expanded lowest quantile
            delta_expanded = np.expand_dims(delta, axis=-1)  # Shape: (samples, time, nowcasting_steps, 1)
            quantile_outputs.append(output_lowest_quant_expanded + delta_expanded)
        outputs = np.concatenate(quantile_outputs, axis=-1)
    ...
    def _organise_data(self, X, y):
        fit_data = {}
        for fold in tqdm(self.split_idx_.keys(), disable=~self.verbose):
            fit_data[fold] = {}
            for chunk in ["train", "valid"]:
                fit_data[fold][chunk] = {"X": {}, "y": resampled_y_df.iloc[tft.split_idx_[fold][chunk]]}
                for Xk, Xv in X.items():
                    fit_data[fold][chunk]["X"][Xk] = []
                    for y_date in resampled_y_df.index[self.split_idx_[fold][chunk]]:
                        date_lim = self.last_avail_dates_[f"LastAvail{Xk}"][y_date]
                        try:
                            to_pad = Xv[:date_lim].values
                        except KeyError:
                            to_pad = np.zeros((1, 1))
                        fit_data[fold][chunk]["X"][Xk].append(to_pad)
                    fit_data[fold][chunk]["X"][Xk] = keras.utils.pad_sequences(fit_data[fold][chunk]["X"][Xk], maxlen=self.lags[Xk], dtype=np.float32)
        return fit_data
    ...
    def fit(self, X:dict, y:dict):
        ...
        # first we find all data with higher frequencies then `y`
        ...
        all_freqs  = list(X.keys()) + list(y.keys())
        ...
        # TODO: construct tscv
        # TODO: actually fit model

tft = TFT()
X_complicated = {"D": df_oil_d, "W": df_oil_d.resample("W").bfill(), "QE": df_oil_d.resample("QE").bfill()}
tmp_out = tft.fit(X={"D": df_oil_d}, y={"ME": df_infl_m})
tmp_out
```
...
```{python}
tft = TFT(fit_args=fit_args, time_feat=None)

y_fit = pd.DataFrame(y_train_split["fold_4"]["train"], index=[i.name for i in y_train_split["fold_4"]["train"]])

tft.fit(X=vsn_X_train, y={"m": y_fit}, valid_data=(vsn_X_valid, np.array(y_train_split["fold_4"]["valid"])))
```

```{python}
#| label: fig-loss_tft
#| fig-cap: Losses of the TFT model

pd.DataFrame(tft.history_.history).plot()
```
