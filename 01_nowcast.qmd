...
class TFT(BaseEstimator, RegressorMixin):
    ...
    def _build_model(self):
        hist_inputs = {k: keras.layers.Input(shape=(None,1), name=f"HistInput__freq_{k}") for k in self.freq_rank.keys()} # should I change this shape to (None, None)???
        encoded_cont_inputs = {
            k: keras.layers.TimeDistributed(
        ...
        fut_inputs = {f: keras.layers.Input(shape=(None, 1), name=f"FutInput__freq_{k}") for k in higher_freqs}
        ...
        future_dec = keras.layers.TimeDistributed(keras.layers.Dense(units=self.d_model, use_bias="False"))(future_dec)
