...
def create_data(X, y, maxlags=maxlags, tscv=TimeSeriesSplit(n_splits=5), timedim=True):
...
One particular type of RNN that has proven to be very successful in practice is the long short-term memory (LSTM) model, due to @hochreiter1997long. It is actually a combination of four different layers, of which three are actually gates. These layers are built in a specific way. Here's how:
...
class TemporalVariableSelection(keras.Layer):
    ...
    def build(self, input_shape):
        ...
        self.batch_size, self.time_steps, self.num_input_vars, self.d_model = input_shape
    ...
    def call(
        ...
        c_s = keras.ops.expand_dims(static_context, axis=1) if context else None
...
# encode each variable into R^dim before passing to the VSN
encoded_series = {f: keras.layers.TimeDistributed(keras.layers.Dense(dim, activation="relu", name=f"encoding_{f}"))(inputs[f]) for f in inputs.keys()}
encoded_series = {k: keras.ops.expand_dims(v, axis=2) for k, v in encoded_series.items()} # add a `variable` dimension in axis=2. axis=3 reserved for the embedding dimension
encoded_series = keras.ops.concatenate([v for v in encoded_series.values()], axis=2)
# VSN
####
# TemporalVariableSelection(d_model=dim)(encoded_series)
####
flattened = keras.layers.Reshape((-1, len(inputs)*dim))(encoded_series)
grn_flattened, _ = GatedResidualNetwork(
            d_model=dim,
            output_size=len(inputs),
            dropout_rate=0.,
            use_time_distributed=True,
        )(flattened)
weights = keras.layers.Activation("softmax")(grn_flattened)
...
```{python}
#| label: train the lstm with multihead attention
X_train_split, y_train_split = create_data(X=X_train, y=y_train, maxlags=maxlags)
...
#| fig-cap: Losses of the model with intepretable multi-head attention
