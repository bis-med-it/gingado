...
graph TD;
    ...
    D1 --> E1[Monthly X_data];
    D1 --> E2[Daily X_data];
    
    C2 --> F1[X_data];
    C2 --> F2[Y_data];
    
    F1 --> G1[Monthly X_data];
    F1 --> G2[Daily X_data];
    ...
    I1 --> J1[Monthly X_data];
    I1 --> J2[Daily X_data];
    
    H2 --> K1[X_data];
    H2 --> K2[Y_data];
    
    K1 --> L1[Monthly X_data];
    K1 --> L2[Daily X_data];
    ...
    N1 --> O1[Monthly X_data];
    N1 --> O2[Daily X_data];
    
    M2 --> P1[X_data];
    M2 --> P2[Y_data];
    
    P1 --> Q1[Monthly X_data];
    P1 --> Q2[Daily X_data];
```

But because each fold is a sequentially longer window, we get the following data points:
...
input_monthly = keras.layers.Input(shape=(None,1), name="Inflation (m)")
input_daily = keras.layers.Input(shape=(None,1), name="Oil (d)")
all_inputs = keras.layers.Concatenate()([input_monthly, input_daily])
out = keras.layers.Dense(units = d, activation="relu")(all_inputs)
out = keras.layers.Dense(units=1)(out)

nn = keras.Model(
    inputs=[input_monthly, input_daily], 
    outputs=out
)
nn.summary()
```
