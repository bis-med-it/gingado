...
```{python}
#| label: embedding the temporal features

dim = 16

# Step 1: Create input layers for each categorical variable
inputs = {
...
# Here, we use Concatenate to merge all embeddings into a single tensor
combined = keras.layers.Average()(list(embedded_layers.values()))

# Step 4: Optionally, you can flatten the combined embeddings if necessary
# combined = tf.keras.layers.Flatten()(combined)

# Step 5: Add additional dense layers for further processing
...
for feature_index in range(embed_ts[0].shape[1]):
    plt.plot(embed_ts[0][-window_months:, feature_index], label=f'Feature {feature_index + 1}')
...
plt.figure(figsize=(10, 6))
for feature_index in range(embed_ts[0].shape[1]):
    plt.plot(embed_ts[1][-window_months:, feature_index], label=f'Feature {feature_index + 1}')
...
# TO-DO: include average of embeddings
plt.figure(figsize=(10, 6))
for feature_index in range(embed_ts[0].shape[1]):
    plt.plot(embed_ts[1][-window_months:, feature_index], label=f'Feature {feature_index + 1}')
