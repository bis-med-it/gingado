...
class TFT(BaseEstimator, RegressorMixin):
    def __init__(
        ...
        time_feat:str|list|None="all",
        compile_args={"loss": quantile_loss},
    ...
    def _build_tft_model(self):
        # <continuous variables>
        cont_inputs = {k: keras.layers.Input(shape=(None,1), name=k) for k in self.X_freqs_}
        encoded_cont_inputs = {
            k: keras.layers.TimeDistributed(
                keras.layers.Dense(dim),
                name=f"encoding__freq_{k}"
        ...
        # TEMP: expand dims to add a dimension for the number of variables (in this case, one)
        encoded_cont_inputs = {k: keras.ops.expand_dims(v, axis=2) for k, v in encoded_cont_inputs.items()}
        # </continuous variables>
        ...
        if self.time_feat:
            ...
            encoded_cat_inputs = {
                k: keras.layers.Embedding(input_dim=v, output_dim=dim, name=f"embedding__{k}")(cat_inputs[k])
                for k, v in self.vocab_sizes.items()
            }
            ...
            encoded_inputs = encoded_cont_inputs
        # </categorical variables>
        
        #<VSN>
        flattened = {
            k: keras.layers.Reshape(target_shape=(-1, v.shape[-1] * v.shape[-2]), name=f"flat__freq_{k}")(v) # \Xi_t
            for k, v in encoded_inputs.items()
        }
        parallel_variables = {
            k: keras.ops.split(
        ...
        for k, v in combined_features.items():
            lstm = keras.layers.Masking(mask_value=0.0)(v)
            lstm = keras.layers.LSTM(units=dim, return_sequences=True, return_state=False, name=f"LSTM__freq_{k}")(lstm)
            LSTMs[k] = lstm

        mixfreq_attn = keras.layers.Attention()(query=LSTMs["m"], value=LSTMs["d"])
        ...
        # The ReLU activation will yield a non-negative number, thus remaining monotonical where the
        # lowest quantile is the output of `base_output_layer` and the increasingly higher quantiles
        # are the output of `base_output_layer` summed with the output of the elements of `quantile_diff_layers`

        base_output_layer = keras.layers.TimeDistributed(
            keras.layers.Dense(self.num_steps_forecast),
            name="output_lowest_quantile"
        )
        quantile_diff_layers = [
            keras.layers.TimeDistributed(
                keras.layers.Dense(self.num_steps_forecast, activation="relu"),
                name=f"output_quantile_diff_{i}"
            ) 
            for i in range(self.output_len - 1)
        ]
        
        base_quantile = base_output_layer(encoded_series)

        quantile_outputs = [base_quantile]
        for i in range(self.output_len - 1):
            quantile_diff = quantile_diff_layers[i](encoded_series)
        ...
        # </Quantile output - WIP>
        #out = keras.layers.Dense(units=self.num_steps_forecast)(out)

        inputs = cont_inputs | cat_inputs if self.time_feat else cont_inputs
        TFTmodel = keras.Model(
            inputs=inputs,
            outputs=final_output,
            name=self.name
        )
        
        TFTmodel.compile(**self.compile_args)
        return TFTmodel
    
    def fit(self, X, y, valid_data): # needs to take in a dictionary of pandas DataFrame with date in the indices if `time_feat=True`
        
        # <preliminaries>
        # TODO: set up `n_features_in_` for each key in X
        # self.n_features_in_ = 

        self.X_keys_ = X.keys()
        self.X_freqs_ = [f for f in X.keys() if f in ["b", "d", "w", "m", "q", "y"]]# ["B", "D", "W", "M", "Q", "Y"]] # see https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#period-aliases
        self.y_freqs_ = y.keys()
        self.num_steps_forecast = list(y.values())[0].shape[-1]
        # </preliminaries>
        ...
        self.history_ = self.model.fit(x=X, y=y, validation_data=valid_data, **self.fit_args)
        # </build and fit>
    ...
    def plot(self, show_shapes=True, show_layer_names=True, show_layer_activations=True):
