...
`list_all_dataflows` returns a pandas Series by default, which makes it easier to inspect a subset of the catalog instead of printing the entire result:

```{python}

dflows = list_all_dataflows(return_pandas=True)
assert isinstance(dflows, pd.Series)
assert len(dflows) > 0

display(dflows.loc["BIS"].head())
display(dflows[dflows.str.contains("Interest rate", case=False)].head())
```

This format allows users to search `dflows` by source or by the human-readable dataflow name (such as "Interest rate" above). If you only need the dataflow codes, use `list_all_dataflows(codes_only=True, return_pandas=True)`.
...
df = load_SDMX_data(sources={'ECB': 'CISS', 'BIS': 'WS_CBPOL'}, keys={'FREQ': 'D'}, params={'startPeriod': 2003})
