...
gingado provides the `get_timefeat` method to extract these features from a time series:
```{python}
#| output: asis
#| echo: false
...
```{python}	
# For example, using the sdmx data from the previous step
display(df.head())
# Daily data, so we set freq="D"
temporal = get_timefeat(df, freq="D", add_to_df=False)
display(temporal.head())

```

The temporal features can also be integrated directly into the DataFrame by setting the parameter `add_to_df=True`:
```{python}
...
# Supplement weekly data with temporal features:
df_with_timefeat = get_timefeat(df_weekly, freq="W", add_to_df=True)
display(df_with_timefeat.head())

```

It is also possible to supply a list of feature names in case you only want to add a subset of the temporal features:
...
In addition, gingado also provides the `TemporalFeatureTransformer` class, which can be used to transform a DataFrame with a temporal index into a DataFrame with additional features.
