...
@pytest.mark.parametrize(
    "input_freq, expected",
    [
        ("D", tp.Frequency.DAILY),
        ("w", tp.Frequency.WEEKLY),
        ("MS", tp.Frequency.MONTHLY),
        ("qs", tp.Frequency.QUARTERLY),
    ],
)
...
def test_validate_and_get_freq_pandas_compatibility():
    ...
    for freq in tp.Frequency:
        ...
        if freq == tp.Frequency.WEEKLY:
            assert offset.name == "W-SUN"
        elif freq == tp.Frequency.QUARTERLY:
...
@pytest.mark.parametrize(
    ...
    [
        (tp.Frequency.DAILY, 12),
        (tp.Frequency.WEEKLY, 8),
        (tp.Frequency.MONTHLY, 5),
        (tp.Frequency.QUARTERLY, 3),
    ],
)
def test_get_timefeat_add_to_df(
    sample_df: pd.DataFrame, frequency: tp.Frequency, num_columns: int
) -> None:
...
def test_get_timefeat_return_only_features(
    sample_df: pd.DataFrame, frequency: tp.Frequency, num_columns
) -> None:
...
@pytest.mark.parametrize("frequency", list(tp.Frequency))
def test_get_timefeat_invalid_index(frequency: tp.Frequency) -> None:
...
def test_feature_values(sample_df: pd.DataFrame) -> None:
    ...
    result = tp.get_timefeat(sample_df, freq=tp.Frequency.DAILY, add_to_df=False)
...
@pytest.mark.parametrize("frequency", ["D", tp.Frequency.DAILY])
def test_drop_zero_variance_daily(daily_df: pd.DataFrame, frequency: str | tp.Frequency) -> None:
...
@pytest.mark.parametrize("frequency", ["MS", tp.Frequency.MONTHLY])
def test_drop_zero_variance_monthly(
    monthly_start_df: pd.DataFrame, frequency: str | tp.Frequency
) -> None:
...
@pytest.mark.parametrize("frequency", ["QS", tp.Frequency.QUARTERLY])
def test_drop_zero_variance_quarterly(
    quarterly_start_df: pd.DataFrame, frequency: str | tp.Frequency
) -> None:
