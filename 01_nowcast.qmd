...
embedded_layers = {
    k: keras.layers.Embedding(input_dim=v, output_dim=262, name=f"{k}_embedding")(inputs_time[k])
    for k, v in vocab_sizes.items()
}

context = keras.layers.Average()(list(embedded_layers.values()))

gated_features, gate = GatedResidualNetwork(d_model=262, use_time_distributed=False, name="GRN")(inputs_cont, additional_context=context)
